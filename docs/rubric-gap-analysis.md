...
|---|---:|---|---|---|---|
| 創意度 | 25% | 原創圖譜建構、融合機制、可展示 trace | Train-time exact evidence、rolling Query→Skill/Job graph、confidence abstention、candidate quarantine、可點選 trace；歷史 Bedrock pilot 只作對照 | **競賽證據完成** | Production deterministic graph 仍需完整 release gate |
| 技術可行性 | 25% | 可落地；處理 leakage、OOV、曝光偏差與失敗模式 | Unbiased LambdaMART、strictly-earlier-day graph、OOV/cold-start、雙 disjoint confirmation 與 paired bootstrap；新 graph worker 零 Bedrock IAM，corpus inventory 1,218,635/967,377 已驗證 | 接近完成 | 完整 deterministic Neptune import 與 `<800 ms` smoke |
...
1. **完整 deterministic Neptune graph 尚未發布**：200-record Bedrock pilot 是歷史實驗，
...
> 多數搜尋系統把 LLM 放在結果後面寫說明；SkillWeave 的 production graph 則以 exact evidence 與全量統計產生可驗證、可消融、可追溯的技能結構。線上 LLM 只做 Query normalization。
