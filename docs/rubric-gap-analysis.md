...
| 主題切合度 | 20% | 生成式 AI 是必要核心；移除圖譜 NDCG 明顯退化 | 1,993-query locked confirmation：NDCG +1.34%、MRR +1.72%，paired CI 排除 0；仍低於建議 5% | **部分通過** | 用完整 Bedrock graph 提升 query/skill coverage，重新接受預註冊 holdout |
| 完成度 | 10% | 功能順、API 正確、AWS URL、5 分鐘影片、GitHub 可重現 | 本機 UI/API/17 tests、holdout、SAM package 完成；沒有 AWS URL、影片、公開 GitHub | 未完成 | 部署 AWS、跑 judge contract smoke、錄影、release tag |
| AWS Kiro 加分 | +5% | 可驗證採用 Kiro | 已建立 Kiro spec scaffold，但沒有 Kiro 執行證據 | 未取得 | 用 Kiro 開啟 spec、完成一項 tracked task，保留 activity／commit evidence |
