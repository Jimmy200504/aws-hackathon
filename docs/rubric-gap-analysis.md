...
| 完成度 | 10% | 功能順、API 正確、AWS URL、5 分鐘影片、GitHub 可重現 | 本機 UI/API、live graph toggle、portable deterministic SAM package、CI/release gate、SAM runtime smoke、300 秒 1080p 繁中影片、公開 GitHub release 與影片 URL 完成；沒有 AWS URL | 接近完成 | AWS 登入後一鍵部署並做 clean-session smoke |
...
3. **外部交付仍不足**：公開影片與 GitHub release 已完成，但缺 AWS Demo URL 與主辦方統一腳本結果。
