...
# Distinguishes "caller did not configure a vocabulary" (auto-load) from an
# explicit `vocabulary=None`, which selects the legacy string-only contract.
_UNSET: Any = object()
...
class BatchCoalescer:
    ...
    def submit(self, query: str, deadline_seconds: float) -> tuple[StructuredQueryIntent | None, bool]:
...
class BedrockQueryNormalizer:
    ...
    def __init__(
        ...
        *,
        ...
        client: Any = None,
        max_tokens: int = 4000,
        vocabulary: QueryIntentVocabulary | None = _UNSET,
        ...
        self.vocabulary = (
            QueryIntentVocabulary.load() if vocabulary is _UNSET else vocabulary
        )
