...
INPUT_NAME = "userSearchLog_20260601_20260607.csv"
OUTPUT_NAME = "userSearchLog_cleaned.csv"
REPORT_NAME = "cleaning_report.txt"
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Clean the organizer search log into userSearchLog_cleaned.csv. "
            "Required before make quality / make index: the benchmark fixture "
            "reads the cleaned log, and the demo index prefers it."
        )
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help=f"defaults to <data-dir>/{INPUT_NAME}",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help=f"defaults to <data-dir>/{OUTPUT_NAME}",
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=None,
        help=f"defaults to <data-dir>/{REPORT_NAME}",
    )
    args = parser.parse_args()

    input_file = args.input or args.data_dir / INPUT_NAME
    output_file = args.output or args.data_dir / OUTPUT_NAME
    report_file = args.report or args.data_dir / REPORT_NAME
    if not input_file.is_file():
        raise SystemExit(
            f"Missing search log: {input_file}\n"
            "Place the organizer CSVs in data/dataset/ first (see README)."
        )
    ...
    report_lines = [
        ...
        "=" * 60,
        f"",
        f"輸入檔案：{input_file.name}",
        f"輸出檔案：{output_file.name}",
        f"",
        f"總筆數：{stats['total']:,}",
