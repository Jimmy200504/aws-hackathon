...
# Only the Bedrock-derived graph nodes. INTENT_FEATURES also come from a model,
# but they are deliberately not removed here: every checked-in llm_off number
# was measured against this narrower definition, and widening it now would
# silently restate a published figure. Attributing the query-intent family
# needs its own arm, not a redefinition of this one.
...
def baseline_allowed_features(scope: str, manifest: dict) -> set[str]:
    ...
    graph_off keeps text, condition, retrieval and query-intent evidence.
    llm_off keeps every reviewed-graph and behavior feature and removes just the
    Bedrock-derived graph family, which isolates that contribution.
    """
    if scope == "graph_off":
